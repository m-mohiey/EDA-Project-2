...
#Data processing
x<- PM25 %>% filter(fips=="24510") %>% filter(grepl("(?=.*otor)(?=.*ehicle)",Short.Name,perl = T)) %>% select(Emissions,year) %>% group_by(year) %>% summarize_each(funs(sum))


#Plotting 
windows()

plot(x,type = "l",main="Emissions from motor vehicle sources changed from 1999-2008 in Baltimore City")

#writing result to plot5.png device
